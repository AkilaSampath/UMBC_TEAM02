...
*/
        for(i=1;i<argc;i++) {
            if(argv[i][0]!='-' || (argv[i][1]!='v' && argv[i][1]!='f' && 
               argv[i][1]!='a' && argv[i][1]!='b' && argv[i][1]!='n') ||
               (argv[i][1]!='v' && argv[i][2]!='=')) {
                printf("Error: Illegal or incomplete argument.\n");
            ...
            switch(argv[i][1]) {
                case 'v':
                    ...
                    break;
                case 'f':
                    if_sin = atoi(argv[i]+3)-1;
                    if(if_sin<0 || if_sin>1) {
                        printf("Error: Invalid argument for the function.\n");
                        MPI_Abort(MPI_COMM_WORLD,1);
                    }
                    break;
                case 'a':
